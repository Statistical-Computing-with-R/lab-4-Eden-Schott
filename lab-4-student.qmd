...
>>>>>>> 0bacf5281bce6d9e1166ff0a03811721a941d19a
```
...
median_infants <- ca_childcare |>
  ...
  select(mfcc_infant, Region, study_year) |>
  group_by(Region, study_year) |>
  summarise(median_infant = median(mfcc_infant)) |>
  pivot_wider(names_from = study_year, 
...
median_childcare <- ca_childcare |>
  ...
  group_by(Region, study_year) |>
  summarise(median_infant = median(mfcc_infant),
            median_toddler = median(mfcc_toddler),
            median_pre = median(mfcc_preschool)) |>
  pivot_longer(cols = c(median_infant, 
                        median_toddler, 
                        median_pre), 
...
ggplot(median_childcare, aes(x = study_year,
  ...
  geom_smooth(method = "loess",
              aes(group = childcare_type)) +  
  facet_wrap(~ childcare_type, 
             scales = "free_y", 
             nrow = 3) + 
  labs(title = "Change in Full-Time Median Price for Center-Based Childcare",
       x = "Study Year",
       y = "Median Price",
       color = "Region")
